...
describe("Exchange", () => {
  ...
  const feePercent = 10;
  let token1, token2, user1, user2, accounts;

  beforeEach(async () => {
    const Exchange = await ethers.getContractFactory("Exchange");
    const Token = await ethers.getContractFactory("Token");

    token1 = await Token.deploy("Dapp University", "DAPP", tokens(1000000));
  ...
  describe("Withdrawing Tokens", () => {
    ...
    describe("Failure", () => {
      it("fails for locked tokens", async () => {
        // Approve Token
        transaction = await token1
          .connect(user1)
          .approve(exchange.address, amount);
  ...
  describe("Making orders", async () => {
    ...
    describe("Failure", async () => {
      ...
      it("rejects with locked tokens", async () => {
        // Approve Token
        transaction = await token1
          .connect(user1)
          .approve(exchange.address, amount);
  ...
  describe("Order actions", async () => {
    ...
    describe("Filling orders partially (1)", async () => {
      describe("Success", () => {
        beforeEach(async () => {
          // user2 fills order
          transaction = await exchange
            .connect(user2)
            ["fillOrder(uint256,uint256)"]("1", tokens(0.5));
          result = await transaction.wait();
        });
        ...
        it("emits a Trade event", async () => {
          ...
          expect(args.tokenGet).to.equal(token2.address);
          expect(args.amountGet).to.equal(tokens(0.5));
          expect(args.tokenGive).to.equal(token1.address);
          expect(args.amountGive).to.equal(tokens(0.5));
    ...
    describe("Filling orders partially (2)", async () => {
      describe("Success", () => {
        beforeEach(async () => {
          // user2 fills order
          transaction = await exchange
    ...
    describe("Filling orders partially (3)", async () => {
      describe("Success", () => {
        beforeEach(async () => {
          // user2 fills order
          transaction = await exchange
          ...
          transaction = await exchange
            .connect(user2)
            ["fillOrder(uint256,uint256)"]("1", tokens(0.4));
          result = await transaction.wait();
        });
        ...
        it("executes the trade and charge fees", async () => {
          ...
          expect(
            await exchange.balanceOf(token2.address, user2.address)
          ).to.equal(tokens(1.01));
          expect(
            await exchange.balanceOf(token2.address, feeAccount.address)
          ).to.equal(tokens(0.09));
        });
        ...
        it("emits a Trade event", async () => {
          ...
          expect(args.user).to.equal(user2.address);
          expect(args.tokenGet).to.equal(token2.address);
          expect(args.amountGet).to.equal(tokens(0.4));
          expect(args.tokenGive).to.equal(token1.address);
          expect(args.amountGive).to.equal(tokens(0.4));
          expect(args.creator).to.equal(user1.address);
          expect(args.timestamp).to.at.least(1);
        });
      });
    ...
    describe("Filling orders", async () => {
      ...
      describe("Failure", () => {
        it("rejects invalid order ids", async () => {
          const invalidOrderId = 99999;
          await expect(
            exchange.connect(user2)["fillOrder(uint256)"](invalidOrderId)
          ).to.be.reverted;
        });

        it("rejects already filled orders", async () => {
          transaction = await exchange.connect(user2)["fillOrder(uint256)"](1);
          await transaction.wait();

          await expect(exchange.connect(user2)["fillOrder(uint256)"](1)).to.be
            .reverted;
        });
